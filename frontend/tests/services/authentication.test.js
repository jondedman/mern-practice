...
describe("authentication service", () => {
  ...
  expect(result.user).toEqual({ id: "1", fullname: "Testy McTest", avatar: "", email: "test@testEmail.com" });
    });
  ...
  describe("signup", () => {
    test("calls the backend url for a token", async () => {
      const testFullname = "Testy McTest";
      const testEmail = "test@testEmail.com";
      const testPassword = "12345678";
      ...
      await signup(testFullname, testEmail, testPassword);
      ...
      expect(options.method).toEqual("POST");
      expect(options.body).toEqual(
        JSON.stringify({ fullname: testFullname, email: testEmail, password: testPassword })
      );
      expect(options.headers["Content-Type"]).toEqual("application/json");
    });
    ...
    test("returns nothing if the signup request was a success", async () => {
      ...
      const token = await signup(testFullname, testEmail, testPassword);
